...
if(count($host_list)>0){
	for($i=0;$i<count($host_list);$i++){		
			...
			echo "<tr bgcolor=#EEEEEE>";
			echo "<td colspan=7 class='reportdata'><b>Service Uptime Summary : </b>";
			...
			if(count($service_list)>0){
				for($j=0;$j<count($service_list);$j++){					
					echo $service_list->service_info[$j]->description ." (". $service_list->service_info[$j]->port. ") &nbsp; : ".$service_list->service_info[$j]->uptime . "% ";
				}
			}
