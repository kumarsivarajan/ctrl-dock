...
OA_Getopt_Check() {
  ...
  if echo "$argument" | grep -qE '^-'; then
    echo -e "\n'-${option}' option used without argument"
    OA_Usage
  fi
}
...
OA_LDAP_Audit() {
  if [ -n "$local_domain" ]; then
    ...
    #  Adjust the below value if your ldap query fails due to memory issues. 1000 is
    #+ the AD default but it can be changed.
    oa_pr_size=1000
...
while getopts hnakqLDIRSb:c:d:l:t:w:N:i:p:P:u:s:y:o:A:H:U: options; do
  case $options in
    a)  opt_syslog="yes";;
    A)  if [ ! -e "$OPTARG" ]; then
          echo -e "\nThe samba credentials file doesn't exist: \"${OPTARG}\"\n"
          exit 1
        fi
    ...
    b)  OA_Getopt_Check "$options" "$OPTARG"
      ...
      ;;
    c)  if [ ! -e "$OPTARG" ]; then
          echo -e "\nThe specified config file doesn't exist: \"${OPTARG}\"\n"
          exit 1
        fi
    ...
    H)  OA_Getopt_Check "$options" "$OPTARG"
      ...
      ;;
    i)  if [ ! -e "$OPTARG" ]; then
          echo -e "\nThe specified input file doesn't exist: \"${OPTARG}\"\n"
          exit 1
        fi
    ...
    l)  if [ -n "$OPTARG" ] && ! echo "$ARGUMENT" | grep -qE '^-'; then
          if $(touch "$OPTARG"); then
            ...
            echo -e "\nUnable to write to logfile location: \"${OPTARG}\"\n"
            exit 1
          fi
        fi
        opt_log_config="y"
    ...
    p)  OA_Getopt_Check "$options" "$OPTARG"
      ...
      ;;
    P)  if [ ! -e "$OPTARG" ]; then
          echo -e "\nThe specified package file doesn't exist: \"${OPTARG}\"\n"
          exit 1
        fi
        opt_packages=$OPTARG
      ;;
    ...
    t)  OA_Getopt_Check "$options" "$OPTARG"
        ...
        case $OPTARG in
          ...
          *)  echo -e "\nUUID type needs to be mac or uuid."; OA_Usage;;
        esac
      ;;
    ...
    w)  if ! echo "$OPTARG" | grep -qE '^//.*\.vbs'; then 
          echo -e "\nUNC path needs to be in form //server/share/audit.vbs\n"
          OA_Usage
        fi
      ...
      ;;
    y)  if [ ! -e "$OPTARG" ]; then
          echo -e "\nThe LDAP password file doesn't exist: \"${OPTARG}\"\n"
          exit 1
        fi
        opt_ad_pass="$OPTARG"
      ;;
...
if [ -n "$opt_unc_path" -a -z "$opt_smb_credentials" ] | [ -n "$opt_smb_credentials" -a -z "$opt_unc_path" ]; then
  echo -e "\nWindows audits require the 'w' and 'A' options"
  OA_Usage
elif [ -n "$opt_audit_local" ]; then
  if [ -z "$opt_offline" ]; then
    echo -e "\nLocal audits require the 'o' switch"
    OA_Usage
  elif [ -z "$opt_web_url" ] && [ "$opt_offline" = "on" ]; then
    echo -e "\nLocal online audits require the 'U' option"
    OA_Usage
  fi
elif [ -n "$opt_audit_remote" ]; then
  if [ -z "$opt_strcomputer" ]; then
    echo -e "\nRemote audits require the 's' option"
    OA_Usage
  elif [ -z "$opt_type" ] && [ -z "$opt_web_url" ]; then 
    echo -e "\nRemote Linux audits require the 'U' option"
    OA_Usage
  fi
  ...
  if [ -z "$opt_ldap_uri" ]; then
    echo -e "\nMissing the LDAP URI"
    echo -e "Domain audits require the 'H' option"
    OA_Usage
  elif [ -z "$opt_type" ] && [ -z "$opt_web_url" ]; then
    echo -e "\nLinux domain audits require the 'U' option"
    OA_Usage
...
if [[ $UID -ne 0 ]]; then
  if [ -z "$opt_root" ]; then
    echo -e "\nThis script should normally be run with root priveleges (If auditing locally). See usage..."
    OA_Usage
  fi
fi
...
if [ -n "$opt_audit_local" ]; then
  ...
  if [ -e "$config_path" ]; then
    ...
    echo -e "\n$config_path not found. Using default settings. See the resulting text file when complete and submit manually.\n"
    verbose="y"
    online="n"
    software_audit="y"
    strComputer="."
    ...
    if [ "$input_file" != "" ] && [ -e "$input_file" ]; then
      ...
      if [ -z "$opt_type" ]; then
        ...
        if [ -z "$opt_ssh_user" ]; then
          echo "\nYou need to specify a default SSH user when doing remote Linux audits"
          OA_Usage
        fi
        OA_Audit_Log "" "log_header"
	  ...
	  echo "\nYou need to specify a samba credentials file for Windows audits"
          OA_Usage
        fi
        OA_Audit_Log "" "log_header"
        OA_Audit_Log "" "log_start_success" "input_file" "$(wc -l "$input_file" | $oa_cut -d" " -f1)"
...
for i in $($oa_ifconfig -a | $oa_awk '/^[A-Za-z0-9]+/{print $1}' | $oa_sort); do
  ...
  if [ -e /var/run/dhclient.${net_connection_id}.pid ] && [ -d /var/lib/dhcp3 ]; then
    dhcp_info_dump=$($oa_awk 'BEGIN{RS = "\{"}; NR == 3' /var/lib/dhcp3/dhclient.${net_connection_id}.leases \
      2> /dev/null | $oa_sed '/^$/d;/}/d;s/^[ \t]*//')
    net_dhcp_server=$(echo "$dhcp_info_dump" | $oa_awk '/dhcp-server/{print $3}' | $oa_sed 's/\;//')
    ...
    net_dhcp_lease_expires=$(echo "$dhcp_info_dump" | $oa_awk '/expire/{print $3 $4}' \
      | $oa_sed 's/\///g;s/\://g;s/\;//g')
    net_service="dhclient"
    net_dhcp_enabled="True"
  elif [ -e /var/run/dhcpcd.${net_connection_id}.pid ] && [ -d /var/lib/dhcpcd ]; then
    net_service="dhcpcd"
    net_dhcp_enabled="True"
  fi 
...
if [ "$online" = "yesxml" ] || [ "$online" = "ie" ]; then
  if [ -z "$oa_wget_missing" ]; then
    ...
    echo -e "\nMissing wget. See '$ReportFile' and submit manually.\n"
  fi
else
  OA_Trace "\nResults have been stored in file '$ReportFile'.\n"
fi
