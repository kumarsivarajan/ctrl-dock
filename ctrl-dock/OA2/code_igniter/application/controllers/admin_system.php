...
class Admin_system extends MY_Controller {
	...
	function add_system() {
		if (!isset($_POST['AddSystem'])) {
			...
			if (!isset($details->man_os_group)) { $details->man_os_group = ''; }
			if (!isset($details->man_os_group_2)) { $details->man_os_group_2 = ''; }
			if (!isset($details->man_os_family_typed)){ $details->man_os_family_typed = ''; }
			if (!isset($details->man_os_name_2)) { $details->man_os_name_2 = ''; }
			...
			$details->last_seen_by = 'web form';
			if ($details->icon == '') { $details->icon = $details->man_type; }
