...
if (is_dir($dir)) {
   if ($dh = opendir($dir)) {
       while (($file = readdir($dh)) !== false) {
           ...
           if (substr($file, -4, 4) == ".txt"){ $files[$count] = $file; $count++;}
       }
       closedir($dh);
   }
}
...
if ($verbose == "y"){echo "<tr><td>" . $sql . "</td></tr>\n";}
...
//Output result
echo "<tr><td>Page was generated in " . round($totaltime,2) . " seconds !</td></tr>\n";
echo "<tr><td>You should now remove these files.</td></tr>\n";
echo "</table>\n";
exit;
...
function insert_startup ($split) {
    ...
    $extended = explode('^^^',$split);
    $startup_caption = addslashes(trim($extended[1]));
    $startup_command = addslashes(trim($extended[2]));
    $startup_description = addslashes(trim($extended[3]));
    $startup_location = addslashes(trim($extended[4]));
    $startup_name = addslashes(trim($extended[5]));
    $startup_user = addslashes(trim($extended[6]));
...
function insert_service ($split) {
    ...
    $extended = explode('^^^',$split);
    $count = $count + 1;
    $service_description = addslashes(trim($extended[1]));
    $service_display_name = addslashes(trim($extended[2]));
    $service_name = addslashes(trim($extended[3]));
    $service_path_name = addslashes(trim($extended[4]));
    $service_started = addslashes(trim($extended[5]));
    $service_start_mode = addslashes(trim($extended[6]));
    $service_state = addslashes(trim($extended[7]));
...
function insert_bho ($split) {
    ...
    $extended = explode('^^^',$split);
    $bho_code_base = addslashes(trim($extended[1]));
    $bho_status = addslashes(trim($extended[2]));
    $bho_program_file = addslashes(trim($extended[3]));
...
function insert_software ($split) {
    ...
    $software_name = addslashes(trim($extended[1]));
    $software_version = addslashes(trim($extended[2]));
    $software_location = addslashes(trim($extended[3]));
    $software_uninstall = addslashes(trim($extended[4]));
    $software_install_date = addslashes(trim($extended[5]));
    $software_publisher = addslashes(trim($extended[6]));
    $software_install_source = addslashes(trim($extended[7]));
    $software_system_component = addslashes(trim($extended[8]));
    $software_url = addslashes(trim($extended[9]));
    $software_comments = addslashes(trim($extended[10]));
    ...
    if (mysql_affected_rows() == 0) {
      ...
      $sql .= "'$software_location','$software_uninstall','$software_install_date','$software_publisher',";
      $sql .= "'$software_install_source','$software_system_component','" . addslashes($software_url) . "','$software_comments','$count','$timestamp','$timestamp')";
      $result = mysql_query($sql) or die ('Insert Failed: ' . mysql_error() . '<br />' . $sql);
      if ($verbose == "y"){echo $sql . "<br />\n\n";}
      if ($verbose == "y"){echo "Affected Rows: " . mysql_affected_rows() . "<br />\n\n";}
    } else {}
