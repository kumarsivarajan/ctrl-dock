...
$sql.=" from rca_approval_history where activity_id='$activity_id' and action in ('ADDED','REJECTED') order by item_order asc limit 1";
...
while ($row= mysql_fetch_row($result)){
	...
	$sub_sql="update rca_approval_history set approver_key='$approver_key',action='$action' where record_index='$record_index'";
	...
	$sub_sql="update rca_master set action='$action' where activity_id='$activity_id'";
	$sub_result = mysql_query($sub_sql);
