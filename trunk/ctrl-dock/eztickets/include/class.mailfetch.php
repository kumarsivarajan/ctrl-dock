...
class MailFetcher {
    ...
    function createTicket($mid,$emailid=0){
		...
		//Giri: First check for specific pattern 
		//TODO: Make this a config pattern later
        if(preg_match ("[INC[0-9]{7}]",$var['subject'],$regs)) {
			$ticket= new Ticket(Ticket::getIdBySubjectPattern($regs[0]),1);
    ...
    function fetchMail(){
        ...
        while($row=db_fetch_array($accounts)) {
            $fetcher = new MailFetcher($row['userid'],$row['userpass'],
