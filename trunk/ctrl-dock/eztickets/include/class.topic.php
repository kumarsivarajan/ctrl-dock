...
class Topic {
    ...
    function load() {
        ...
        if(($res=db_query($sql)) && db_num_rows($res)) {
			...
			$this->parent_topic_id=$info['parent_topic_id'];
			$this->ticket_type_id=$info['ticket_type_id'];
    ...
    function save($id,$vars,&$errors) {
        ...
        if($id && $id!=$vars['topic_id'])		
            $errors['err']='Internal error. Try again $id';
        ...
        */    
        ...
        if(!$errors) {
            $sql='updated=NOW(),topic='.db_input(Format::striptags($vars['topic'])).
				 ...
				 ',parent_topic_id='.db_input($vars['parent_topic_id']).
				 ',ticket_type_id='.db_input($vars['ticket_type_id']).
