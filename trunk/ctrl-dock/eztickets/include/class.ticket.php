...
class Ticket{
    ...
    function load($id) {
	    ...
	    $this->close_location = $row['close_tkt_location'];	
            $this->row=$row;
	...
	if($this->close_location == 'locally'){
	  $this->close_location = 'on-site';
	}
    ...
    //Location helper
    function setCloseLocation($location){
	$update_sql = 'UPDATE '.TICKET_TABLE.' SET close_tkt_location='.db_input($location).' WHERE ticket_id='.db_input($this->getId());
	return (db_query($update_sql)  && db_affected_rows())?true:false;		
    }		
    ...
    function close(){
		...
		$sql="select subject,dept_id from isost_ticket where ticket_id='$ticket_id'";
		$result = mysql_query($sql) or die(mysql_error());
		$row = mysql_fetch_array($result);
		$ticket_subject=$row['subject'];
		$deptId=$row['dept_id'];
		
		$sql="select response from isost_ticket_response where ticket_id='$ticket_id' order by response_id DESC LIMIT 1";
		$result = mysql_query($sql) or die(mysql_error());
		$row = mysql_fetch_array($result);
		
		$closing_comments=$row['response'];
		
		$sql = 'select a.email from isost_staff a where a.close_tkt_not=1 and a.isactive=1 and a.dept_id='.$deptId;
        $result = mysql_query($sql) or die(mysql_error());
        ...
        while($row = mysql_fetch_array($result)){
			...
			$subject = "Ticket [#$ticketNumber] : $ticket_subject : Closed";
			$message = "The ticket with #$ticketNumber has been closed with the following response \n\n $closing_comments \n\n";
		...
		// Close any child tickets if they exist
		
		$sql="select ticket_id from isost_merge where track_id='$ticket_id'";
		$result = mysql_query($sql);
		$row = mysql_fetch_row($result);
		$merged_tickets=explode(",",$row[0]);
			
		for ($i=0;$i<count($merged_tickets);$i++){
			$sql="update isost_ticket set status='closed',isoverdue=0,duedate=NULL,updated=NOW(),closed=NOW() where ticket_id='".$merged_tickets[$i]."'";
			$result = mysql_query($sql);
		}
    ...
    function transfer($deptId){
        ...
        */
		$sql = 'select a.email,b.dept_name from isost_staff a,isost_department b where a.dept_id=b.dept_id and a.new_tkt_not=1 and a.isactive=1 and a.dept_id='.$deptId;
        $result = mysql_query($sql) or die(mysql_error());
        ...
        while($row = mysql_fetch_array($result)){
			...
			$message = "Your department ($deptName) has been assigned a ticket with No. #$ticketNumber . Kindly address the ticket accordingly.\n\n";
    ...
    function postResponse($msgid,$response,$signature='none',$attachment=false,$mail_cc,$canalert=true,$time_hh=0,$time_mm=0){
        ...
        if(db_query($sql) && ($resp_id=db_insert_id())):
			...
			//Checking whether it is system or User
			$sysornot = $thisuser->getName();
			$cmpstr=strcasecmp($sysornot,'system');
			if($cmpstr != 1){
				...
				if(($resp=db_query($sql)) && db_num_rows($resp) && list($subj,$body)=db_fetch_row($resp)){
					...
					if($email && $email->getId()) {
						...
						$email->send($this->getEmail(),$subj,$body,$file,$cc_mail);
					}
				}
				else{
   ...
   /*============== Functions below do not require an instance of the class to be used. To call it use Ticket::function(params); ==================*/
	...
	function getIdBySubjectPattern($pattern, $limit=1){
		if (!$pattern){
			return 0;
		}
		$lim='';
		...
		$sql = 	"SELECT ticket.ticket_id FROM ". TICKET_TABLE . " ticket ".
				"WHERE ticket.subject LIKE '%".$pattern."%' $lim";
		$id=0;
		if(($res=db_query($sql)) && db_num_rows($res))
            list($id)=db_fetch_row($res);
    ...
    function update($var,&$errors) {
		 ...
		 $fields['assetId']  = array('type'=>'int');
		 $fields['tickettype']  = array('type'=>'int');
        ...
        if($var['topicId'] && ($topic= new Topic($var['topicId'])) && $topic->getId()) {
         ...
         if(!$errors){
             $sql='UPDATE '.TICKET_TABLE.' SET updated=NOW() '.
				  ...
				  ',asset_id='.db_input($var['assetId']).
				  ',ticket_type_id='.db_input($var['tickettype']).
    ...
    function create($var,&$errors,$origin,$autorespond=true,$alertstaff=true) {
       ...
       /* Coders never code so fully and joyfully as when they do it for free  - Peter Rotich */
        ...
        if(strcasecmp($origin,'web')==0) { //Help topic only applicable on web tickets.
			...
			$fields['assetId']   = array('type'=>'int',      'required'=>0, 'error'=>'Asset ID required');
			$fields['tickettype']   = array('type'=>'int',      'required'=>0, 'error'=>'Ticket Type required');
		...
		//new asset_id included in the sql query
                ...
                if(($resp=db_query($sql)) && db_num_rows($resp) && list($subj,$body)=db_fetch_row($resp)){
                    ...
                    if($email && $email->getId()) {
                        ...
                        if($cfg->alertDeptMembersONNewTicket()) {
                            $sql='SELECT staff_id FROM '.STAFF_TABLE.' WHERE onvacation=0 AND new_tkt_not=1 AND dept_id='.db_input($dept->getId());
                            if(($users=db_query($sql)) && db_num_rows($users)) {
                                while(list($id)=db_fetch_row($users))
