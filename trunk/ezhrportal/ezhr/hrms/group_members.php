...
?>
	...
	<tr>
		<td align=left><h2>Manage Profile Members : <?=$group;?></h2></td>
		<td align=right></td>
	</tr>
	</table>
	...
	<tr>
		<td align=center>
			<form method="POST" action="manage_group_member.php?id=<?echo $id;?>&group=<?echo $group;?>&action=add">
			<select size=10 name=member[] class='formselect' style=' height: 300px;' multiple>
			<?php
			    $sql = "select username,first_name,last_name from user_master where account_status='Active' and username not in (select username from ezhr_user_group where group_id='$id') order by username";	
...
<?
	$sql = "select a.username,b.first_name,b.last_name from ezhr_user_group a, user_master b where a.group_id='$id' and a.username=b.username and b.account_status='Active' order by username";	
	$result = mysql_query($sql);	  
	...
	?>
		...
		<tr>	
		<td class="reportdata" colspan=2>This group has no members currently</td>
		</tr>
		</table>
	<?
	}
	if ($num_rows>0){
		...
		<form method="POST" action="manage_group_member.php?id=<?echo $id;?>&group=<?echo $group;?>&action=del">
		<?
			while ($row = mysql_fetch_row($result)){
				echo "<tr>";			
