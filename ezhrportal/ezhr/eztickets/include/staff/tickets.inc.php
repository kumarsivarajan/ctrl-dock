<?php
// function to get help topics
function gethelp($id){
$sql = "select helptopic from isost_ticket where ticket_id = '$id' limit 1";
$sql = mysql_query($sql);
        ...
        if(is_numeric($searchTerm)){
            ...
            if($_REQUEST['stype'] && $_REQUEST['stype']=='FT') { //Using full text on big fields.
                $qwhere.=" AND ( ticket.email LIKE '%$queryterm%'".
							...
							" OR ticket.helptopic LIKE '%$queryterm%'".
...
$qselect = 'SELECT DISTINCT ticket.ticket_id,lock_id,ticketID,ticket.dept_id,ticket.staff_id,helptopic,subject,name,email,dept_name '.
...
<div id='advance' style="display:<?=$basic_display?'none':'block'?>">
 ...
 <input type="hidden" name="a" value="search">
  <table>
    <tr>
        <td>Subject / HelpTopic: </td><td><input type="text" id="query" name="query" value="<?=Format::htmlchars($_REQUEST['query'])?>"></td>
        <td>Dept:</td>
       ...
       <table width="100%" border="0" cellspacing=0 cellpadding=2 class="dtable" align="center">
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?=$class?> " id="<?=$row['ticket_id']?>">
				...
				<td nowrap><?php $id = $row['ticket_id']; gethelp($id);?></td>
